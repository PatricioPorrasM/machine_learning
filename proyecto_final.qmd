...
4.  **Rendimiento de Clasificación Multiclase:** El modelo `LogisticRegression` logró una **AUC-ROC promedio de 0.69** y un **accuracy general del 42%**, pero es significativamente mejor que el azar (que sería un 25% para 4 clases balanceadas). El principal hallazgo es que el modelo muestra un fuerte sesgo hacia la clase Q1 (Bajo). Es bastante bueno identificando los tweets menos tóxicos (Recall del 70%), pero lo hace a costa de ignorar o confundir las otras tres categorías. Es particularmente malo distinguiendo entre las clases medias (Q2 y Q3), donde su rendimiento es apenas superior al azar.
